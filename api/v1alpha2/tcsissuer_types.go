...
type KeyConfig struct {
	// Local represents provision the key locally by the
	// TCS issuer.
	Local *KeyLocalConfig `json:"local"`
	// Remote represents the provisions the key by a remote
	// PKI server using trusted-attestation-controller.
	Remote *RemoteConfig `json:"remote"`
}

type CertConfig struct {
	// Local represents provision the self-singend certificate
	// locally by the TCS issuer.
	Local *CertLocalConfig `json:"local"`
	// Remote represents the provisions the root certificate by
	// a remote PKI server using trusted-attestation-controller.
	Remote *RemoteConfig `json:"remote"`
}

type KeyLocalConfig struct {
	// Algorithm refers to the private key algorithm to use.
	// One of: "rsa", "ecdsa"
	// +kubebuilder:default="rsa"
	Algorithm string `json:"algorithm"`

	// Size refers to the key size in bits
	// For RSA: 2048, 3072 or 4096
	// +kubebuilder:default:3072
	Size string `json:"size"`
}

type CertLocalConfig struct {
	// Duration The requested 'duration' (i.e. lifetime) of the
	// certificate being created.
	Duration *metav1.Duration `json:"duration,omitempty"`
}

type RemoteConfig struct {
	URL string `json:"url"` // tac://<<plugin>>
}
